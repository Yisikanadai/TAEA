function [info, popout] = RL_GWO2(poptemp, info, fit)
    info.l = 1;
    [~, index] = sort(fit);
    ...
    for i = 1:2:round(ps)
        ...
        j = randi([1 len - 1]);
        k = randi([j len]);
        poptemp(i1, :) = [poptemp(i1, 1:j) poptemp(i2, j + 1:k) poptemp(i1, k + 1:end)];
        poptemp(i2, :) = [poptemp(i2, 1:j) poptemp(i1, j + 1:k) poptemp(i2, k + 1:end)];
    end
    popout = poptemp;
    [ps, l] = size(poptemp);
    pm = 0.4;
